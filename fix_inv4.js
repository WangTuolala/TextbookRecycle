...
// Replace ONLY the FIRST occurrence in loadInventoryBooks (line ~1177)
const oldFirst = "async function loadInventoryBooks() {\n        try {\n            const result = await adminApiCall('/inventory/books', 'GET');";
const newFirst = "async function loadInventoryBooks() {\n        try {\n            const result = await adminApiCall('/inventory/books-with-operators', 'GET');";

if (c.includes(oldFirst)) {
    c = c.replace(oldFirst, newFirst);
    console.log('Replaced loadInventoryBooks endpoint');
} else {
    console.log('First occurrence not found');
}

// Fix renderInventoryTable
